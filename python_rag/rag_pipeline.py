...
class RAGPipeline:
    ...
    def list_chunks(self, limit: int = 20) -> list[dict]:
        payload = self.collection.get(limit=limit, include=["documents", "metadatas"])
        ids = payload.get("ids", [])
    ...
    def _build_retrieval_query(question: str, conversation_history: list[dict] | None) -> str:
        ...
        for turn in conversation_history[-2:]:
            prev_question = (turn.get("question") or "").strip()
            prev_answer = (turn.get("answer") or "").strip()
            if prev_question:
                lines.append(f"Previous question: {prev_question}")
            if prev_answer:
                lines.append(f"Previous answer: {prev_answer[:280]}")
    ...
    def answer(
        ...
        if not retrieved.context:
            return {"question": question, "answer": "I don't know.", "sources": [], "chunks": []}
        ...
        answer_text = generate_chat(prompt=prompt) or "I don't know."
        ...
        if normalized in {"i don't know", "i don't know.", "i dont know", "i dont know."} and retrieved.chunks:
            ...
            answer_text = f"{' '.join(extract.split())[:520].strip()} [{top_chunks[0]['source']}]"
